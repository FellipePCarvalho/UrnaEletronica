...
# ESCOLHA // CADASTRAR UM CANDIDATO//
    while escolhaMenu == 0:
        while True:
            print('-' * 25)
            nome = input("Nome: ")
            if all(n.isalpha() or n.isspace() for n in nome):  # identificando se foi digitado apenas letras e espaço
                break  # DIGITAÇÃO CORRETA SAI DO LOOP
            else:
                print('-'*31)
        ...
        while True:
            ...
            if all(p.isalpha() or p.isspace() for p in partido):  # identificando se foi digitado apenas letras e espaço
                break  # DIGITAÇÃO CORRETA SAI DO LOOP
            else:
                print('-'*34)
        ...
        while True:
            try:
                ...
                if not numero in range(0, 99999):  # IDENTIFICANDO SE NUMERO ESTÁ NO RANGE
                    raise ValueError()  # SE N ESTIVER NO RANGE, VAI IDENTIFICAR COMO ERRO
            except ValueError as num:  # SOLICITANDO ENTRADA DE NOVO DADO DEVIDO AO ERRO
                print('-'*33)
...
if escolhaMenu == 3:
    ...
    print(f"{formatacao:6}{'Brancos':17}{'--':16}{'--'}{nulo:12}{'--':>14}{porcentagemBranco:18.2f}")
    print('\n')
